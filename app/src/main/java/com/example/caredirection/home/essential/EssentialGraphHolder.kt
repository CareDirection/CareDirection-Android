...
class EssentialGraphHolder (private val view: View):RecyclerView.ViewHolder(view){
    ...
    var xLabelIngredients2= ArrayList<String>()
    var xLabelIngredients= ArrayList<String>()
    ...
    fun bind(data: RvEssentialGraphData){
            ...
            listData.add(BarEntry(0f,data.percent.toFloat()))
            xLabelIngredients2.add(data.title)
        initLineChart()
        setChart(listData,xLabelIngredients2.toTypedArray()  )
    }
...
//        xAxis.setDrawLabels(false) 이것이 바로 라벨을 지워주는 친구였다 ex) 비타민 D 같은 것들
    ...
    */
    ...
    ll1.lineWidth = 3f
    ll1.enableDashedLine(30f, 20f, 0f)
    ll1.lineColor = ContextCompat.getColor(itemView.context, R.color.colorRed)
    ...
    ll2.lineColor = ContextCompat.getColor(itemView.context, R.color.colorPrimary)
    ll2.enableDashedLine(30f, 20f, 0f)
    ...
    fun setChart(listData:List<BarEntry> , xLabelIngredients: Array<String>) {
        ...
        listColor.add(ContextCompat.getColor(itemView.context, R.color.colorBlueGraph))
        listColor.add(ContextCompat.getColor(itemView.context, R.color.colorRedGraph))
...
//                Log.v("YGYG", vitamins[value.toInt()])
        ...
        dataSet.valueTextColor = ContextCompat.getColor(itemView.context, android.R.color.black)
        ...
        view.rv_chart.setVisibleXRange(1f, 1f) // X에 그려줄 최소, 최대 단위 정하기
